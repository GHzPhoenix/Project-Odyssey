...
app.put("/api/deals/:id", verifyToken, verifyAdmin, (req, res) => {
  const { name, location, activities, start_date, end_date } = req.body;
  db.prepare(
    `UPDATE deals
       SET name=?, location=?, activities=?, start_date=?, end_date=?
     WHERE id=?`
  ).run(name, location, activities, start_date, end_date, req.params.id);
...
request.requestBody({
  ...
  application_context: {
    brand_name: "Travel Odyssey",
    user_action: "PAY_NOW",
    return_url: "http://127.0.0.1:5500/Frontend/membership-success.html",
    cancel_url: "http://127.0.0.1:5500/Frontend/membership-confirmation.html"
  }
  });

  try {
    const order = await ppClient.execute(request);
    const id = order.result.id;
    const approveLink = order.result.links.find(l => l.rel === 'approve').href;

    const returnUrl = `http://127.0.0.1:5500/Frontend/membership-success.html?orderID=${id}`;

    res.json({ orderID: id, approveLink, returnUrl });
  } catch (err) {
...
try {
    ...
    runExec(
      `INSERT OR IGNORE INTO paypal_transactions
         (id, reference_id, amount, currency, create_time)
       VALUES (?, ?, ?, ?, ?)`,
      [
        cap.id,
        pu.reference_id,
