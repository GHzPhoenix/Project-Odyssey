...
// in index.js, after your bookings & inquiries:

// Purchase or renew membership by email (no auth token needed)
app.post("/api/membership/purchase", (req, res) => {
  ...
  // find user by email
  const user = runAll("SELECT id FROM users WHERE email = ?", [email.trim()])[0];
  if (!user) {
    return res.status(404).json({ error: "No user with that email" });
  }
  // compute next expiry (1 year from now or from existing expiry)
  const existing = runAll(
  ...
  if (existing) {
    runExec(
      ...
      [plan, isoDate, user.id]
    );
  } else {
    runExec(
      "INSERT INTO memberships (user_id, membership_type, membership_expires) VALUES (?, ?, ?)",
      [user.id, plan, isoDate]
    );
  }
  res.json({ membership_type: plan, membership_expires: isoDate });
});
