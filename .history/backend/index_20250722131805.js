...
app.get("/api/users/summary", verifyToken, verifyAdmin, (req, res) => {
  try {
    const row = db.prepare("SELECT * FROM user_summary").get();
    res.json(row);
  } catch (err) {
...
app.get("/api/users/all", verifyToken, verifyAdmin, (req, res) => {
  try {
    const users = db.prepare("SELECT * FROM user_list").all();
    res.json(users);
  } catch (err) {
...
// at the top of your file, after you set up `app`, `db`, and your auth middleware:

// ——————————————————————————————————————
// 1) SUMMARY: total, guests, active & cancelled counts
...
// ——————————————————————————————————————
// 2) ACTIVE BOOKINGS: list all non–cancelled, newest first
