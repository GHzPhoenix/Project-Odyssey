...
// CANCEL (soft delete) — users can cancel their own booking
app.patch(
  ...
  async (req, res) => {
    try {
      ...
      return res.json({ message: "Booking cancelled", cancelled_at: now });
    } catch (err) {
...
app.delete(
  ...
  const booking = db
    .prepare(
      `SELECT id, user_id, destination, start_date, end_date, guests, created_at, cancelled_at
       FROM bookings
       WHERE id = ?`
    )
    .get(bookingId);
